    ...
    <div class="page-inner-md">
      ...
      <div v-if="notifications.length" class="space-y-3">
        <div v-for="n in notifications" :key="n.id"
          class="card notification-item" :class="{ unread: !n.read_at }"
          @click="markAsRead(n)">
...
onMounted(async () => {
  try { const r = await raqiAPI.notifications.list(); notifications.value = unwrap(r); }
  catch (e) { console.error(e); }
});

const markAsRead = async (n) => {
  if (n.read_at) return;
  try { await raqiAPI.notifications.markRead(n.id); n.read_at = new Date().toISOString(); }
  catch (e) { console.error(e); }
};
</script>
